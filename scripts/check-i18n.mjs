...
for (const required of ["Page ${current} of ${total}", "Seite ${current} von ${total}", "Your position in this lecture", "Deine Position in dieser Lecture", "Page in the lecture guide", "Seite im Lecture-Guide", "step===index", "is-current"]) if (!conceptPositionRenderer.includes(required)) throw new Error(`concept position renderer: missing ${required}`);
for (const required of ["conceptIndex+1", "lecture.concepts.length", "conceptLecturePositionMarkup(c,lectureId)"]) if (!conceptRenderer.includes(required)) throw new Error(`concept renderer: prominent lecture position is missing ${required}`);
...
for (const required of ["Next page · ${index+2} of ${guide.concepts.length}", "Nächste Seite · ${index+2} von ${guide.concepts.length}", "First page · 1 of ${guide.concepts.length}", "Erste Seite · 1 von ${guide.concepts.length}"]) if (!conceptContinuationRenderer.includes(required)) throw new Error(`concept continuation: page number is missing ${required}`);
