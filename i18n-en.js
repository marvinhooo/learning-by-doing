window.CS336_EN = Object.freeze({
    ...
    "a1": {
      ...
      "missions": [
        ...
        },
        {
          ...
          "labs": ["decode-sampling", "attention", "evaluation"]
        }
      ],
      "done": [
      ...
      "missions": [
        {
          ...
          "labs": ["decode-sampling", "evaluation", "answer-parsing"]
        },
        {
          "id": "pg-math",
  ...
  "labs": {
    ...
    "decode-sampling": {
      "title": "Decoding: temperature, top-p, and the silent bug",
      "desc": "Send the same eight logits through five implementations of equations (23) and (24) — the correct one and four that throw nothing. You read off which tokens the nucleus set contains, how far the output drifts from the correct distribution, and in which state a test can see the bug at all. Exactly Problem (decoding) from A1, and the sampling setting under which A5 produces every rollout.",
      "mental": "Between the model's logits and the token that ends up in the text sit three decisions that have nothing to do with the weights. Temperature scales the spacing of the logits before softmax turns them into probabilities — it belongs in the exponent because softmax responds exponentially to spacing. Top-p then truncates the tail, and not by a fixed count but by probability mass: what is kept is the smallest set whose mass reaches p. And because a subset of a distribution is no longer a distribution, you have to divide by the mass you kept. Each of these three lines is a one-liner, each has an obvious bug, and none of those bugs raises an exception or violates a shape. The model is unchanged throughout — the same weights produce fluent text, endless repetition, or a group of eight identical rollouts depending on these three lines.",
      "formula": "softmax(v, τ)_i = exp(v_i/τ) / Σ_j exp(v_j/τ)  ·  V(p) = smallest set with Σ_{j∈V(p)} q_j ≥ p  ·  P(x_{t+1}=i | q) = q_i / Σ_{j∈V(p)} q_j for i ∈ V(p), otherwise 0  ·  H = −Σ p_i log₂ p_i  ·  P(all G rollouts identical) = p_max^G",
      "symbols": [["v","The model's logit vector at exactly one position, i.e. the last row of the output of shape [B, T, V] and therefore [B, V]. Only this single row is decoded."],["τ","Temperature. It divides the logits before softmax. Below one it widens the spacing and sharpens the distribution, above one it flattens it. τ → 0 must be handled as its own greedy path, because the division is otherwise undefined."],["q","The distribution after temperature and softmax. V(p) is formed on it — not on the unscaled one."],["p","Threshold on cumulative probability mass, not on the number of tokens. p = 1.0 means: truncate nothing."],["V(p)","The smallest index set whose mass reaches or exceeds p. The token that crosses the threshold belongs to it — otherwise the mass would be guaranteed to fall below p, and for a very confident model the set would even come out empty."],["Renormalization","The division by Σ_{j∈V(p)} q_j. Without it the output sums to the kept mass, and every log-probability is shifted by −ln(mass)."],["H","Entropy of the output distribution in bits. It measures how much randomness is left after temperature and truncation: 0 bits means deterministic, 3 bits means eight equally likely tokens."],["G","Rollout group size in A5, there 8. With p_max the probability of the leading token, p_max^G is the probability that all G rollouts pick the same one at this position."]],
      "observe": "Set p = 0.8 and τ = 1.0 and read the |V(p)| column under “The same threshold, four distributions”: 1, 3, 5, 7 — the same line of code, four set sizes. Then hold the context “Two nearly equal candidates” and p = 0.9 fixed and switch τ from 0.5 to 1.5: the set grows from 2 to 6 although p never moved. Now switch to “Temperature after the softmax” and step through τ once more — every number stays put. Set “Off-by-one” in the Paris context at p = 0.8: no token is left. And with “Renormalization forgotten”, read the two lines side by side: distance to the correct distribution 0.0000, sum of the output below one.",
      "misconception": "The three most common errors each appear as their own variant in this lab. First: that temperature is a knob on the probabilities — it belongs in the exponent, and the same division applied after the softmax cancels out completely. Second: that top-p keeps a fixed number of tokens — p is a mass threshold, and how many tokens it takes to reach it is decided by the shape of the distribution. Third: that a sampling test covers these lines — torch.multinomial normalizes internally, so a missing renormalization stays completely invisible while drawing and only hits the log-probabilities. And none of the four bugs produces an exception, a shape violation, or a NaN.",
      "transferQuestion": "A5 produces every GRPO group with G = 8 rollouts at temperature 1.0 and top_p = 1.0 — i.e. by sampling rather than greedily. Take the Paris context as a model of a position where the model is confident, and read the line “Probability that all 8 rollouts pick the same token here” at τ = 1.0 and at τ → 0. What happens to the rewards of a group in which all eight rollouts are identical, and what follows for the advantages GRPO forms from them?",
      "transferAnswer": "At τ → 0 that line reads 100.0 %: greedy delivers eight identical rollouts, and identical rollouts earn identical rewards. GRPO normalizes rewards within the group, so the mean becomes exactly the shared reward, every deviation from it becomes zero, and therefore every advantage becomes zero. The loss is then zero as well, and so is the gradient — the group cost 8 rollouts and contributes exactly nothing to the update. At τ = 1.0 the same position still reads 70.2 %, so even there seven groups in ten make the same choice; only across the many positions of an answer and across prompts of differing difficulty does the spread arise from which GRPO draws any signal at all. That is why the sampling temperature in A5 is not a detail of text quality but the knob that decides whether a group carries a learning signal — and why the handout says sampling_temperature = 1.0 rather than greedy. The same arithmetic also explains why a prompt the model always answers correctly, or always incorrectly, disappears from the group statistics: the problem is not the reward but its variance within the group. To see this in the log, record the group standard deviation alongside the reward, plus the share of groups whose standard deviation is zero."
    },
    "inference-budget": {
  ...
  "ui": {
    "Was gesampelt wird": "What is being sampled",
    "Kontext": "Context",
    "Temperatur τ": "Temperature τ",
    "Top-p": "Top-p",
    "Implementierung": "Implementation",
    "Vier falsche Implementierungen stehen neben der korrekten, und keine davon wirft einen Fehler. Sage vor jedem Wechsel voraus, welche Zeile des Ledgers kippt – und ob der aktuelle Zustand den Fehler überhaupt sehen kann.": "Four wrong implementations sit next to the correct one, and none of them raises an error. Before every switch, predict which row of the ledger flips — and whether the current state can see the bug at all.",
    "Alles direkt aus den angezeigten Zahlen ablesbar.": "All of it readable straight off the displayed numbers.",
    "Interaktives Decoding mit Temperatur und Top-p": "Interactive decoding with temperature and top-p",
    "1. Bei p = 0,8 und τ = 1,0 behält die korrekte Implementierung im Paris-Kontext 1 Token und im Kontext „She said that“ 7. Woran liegt das?": "1. At p = 0.8 and τ = 1.0 the correct implementation keeps 1 token in the Paris context and 7 in the “She said that” context. Why?",
    "An der Form der Verteilung – p ist eine Schwelle auf der Wahrscheinlichkeitsmasse, und wie viele Tokens dafür nötig sind, entscheidet die Verteilung selbst": "Because of the shape of the distribution — p is a threshold on probability mass, and how many tokens it takes to reach it is decided by the distribution itself",
    "An der Länge des Kontexts – längere Prompts erzeugen breitere Verteilungen": "Because of the length of the context — longer prompts produce broader distributions",
    "An einem Fehler: eine korrekte Regel muss bei gleichem p immer gleich viele Tokens behalten": "Because of a bug: at the same p a correct rule must always keep the same number of tokens",
    "2. Die Variante „Temperatur nach dem Softmax“ zeigt für τ = 0,5, τ = 1,0 und τ = 1,5 dieselben Zahlen. Was folgt daraus für die Temperatur-Ablation in A1?": "2. The “temperature after the softmax” variant shows the same numbers for τ = 0.5, τ = 1.0, and τ = 1.5. What follows for the temperature ablation in A1?",
    "Der Regler ist wirkungslos – jede Temperatur misst denselben Lauf, und kein Testlauf meldet das": "The knob does nothing — every temperature measures the same run, and no test run reports it",
    "Die Implementierung ist besonders robust, weil das Ergebnis nicht von τ abhängt": "The implementation is especially robust, because the result does not depend on τ",
    "Der Unterschied ist nur zu klein für die Anzeige und wird bei größerem Vokabular sichtbar": "The difference is merely too small to display and becomes visible with a larger vocabulary",
    "3. „Renormalisierung vergessen“ hat Abstand 0,0000 zur korrekten Verteilung. Wo richtet die Variante trotzdem Schaden an?": "3. “Renormalization forgotten” has distance 0.0000 to the correct distribution. Where does the variant do damage anyway?",
    "Überall dort, wo Wahrscheinlichkeiten gelesen statt Tokens gezogen werden – jede Log-Wahrscheinlichkeit ist um −ln(behaltene Masse) verschoben": "Everywhere probabilities are read rather than tokens drawn — every log-probability is shifted by −ln(kept mass)",
    "Nirgends – ein Abstand von null bedeutet, dass die Implementierung äquivalent ist": "Nowhere — a distance of zero means the implementation is equivalent",
    "Beim Sampling: seltene Tokens werden zu häufig gezogen, weil ihnen zu viel Masse bleibt": "In sampling: rare tokens are drawn too often, because too much mass is left to them",
    "Sicher · ein Token dominiert": "Confident · one token dominates",
    "Der Fall, in dem das Modell die Antwort kennt. Ein Logit liegt vier Punkte über dem nächsten, und schon bei Temperatur 1,0 trägt das erste Token 95,68 % der Masse. Sage voraus, wie viele Tokens Top-p bei p = 0,8 hier übrig lässt – und was ein Sampler tun soll, wenn das erste Token die Schwelle allein schon überschreitet.": "The case where the model knows the answer. One logit sits four points above the next, and already at temperature 1.0 the first token carries 95.68 % of the mass. Predict how many tokens top-p leaves here at p = 0.8 — and what a sampler should do when the first token already crosses the threshold on its own.",
    "Zwei fast gleich starke Kandidaten": "Two nearly equal candidates",
    "Zwei plausible Fortsetzungen liegen 0,1 Logits auseinander, danach fällt das Feld ab. Das ist der Fall, an dem sich ablesen lässt, dass Temperatur und Top-p nicht zwei unabhängige Regler sind: halte p fest und schalte τ durch.": "Two plausible continuations sit 0.1 logits apart, after which the field drops away. This is the case that shows temperature and top-p are not two independent knobs: hold p fixed and step through τ.",
    "Kopf und langer Schwanz": "Head and long tail",
    "Ein moderater Kopf und ein gleichmäßig auslaufender Schwanz – die Form, für die Nucleus Sampling überhaupt erfunden wurde. Beobachte, wie viel Wahrscheinlichkeitsmasse p = 0,9 stehen lässt und wie viele Tokens dafür nötig sind.": "A moderate head and an evenly tapering tail — the shape nucleus sampling was invented for in the first place. Watch how much probability mass p = 0.9 leaves standing and how many tokens that takes.",
    "Flach · das Modell weiß es nicht": "Flat · the model does not know",
    "Acht fast gleichwertige Fortsetzungen; das stärkste Token trägt bei τ = 1,0 nur 15,67 %. Genau hier soll Top-p möglichst wenig wegschneiden – vergleiche die Größe der Nucleus-Menge mit der im Paris-Fall bei derselben Schwelle.": "Eight nearly equivalent continuations; at τ = 1.0 the strongest token carries only 15.67 %. This is exactly where top-p should cut away as little as possible — compare the size of the nucleus set with the one in the Paris case at the same threshold.",
    "τ → 0 · Greedy (eigener Codepfad, keine Division)": "τ → 0 · greedy (its own code path, no division)",
    "τ = 0,5 · schärfer als das Modell": "τ = 0.5 · sharper than the model",
    "τ = 0,8": "τ = 0.8",
    "τ = 1,0 · unverändert (A5-Einstellung)": "τ = 1.0 · unchanged (the A5 setting)",
    "τ = 1,5 · flacher als das Modell": "τ = 1.5 · flatter than the model",
    "p = 0,5": "p = 0.5",
    "p = 0,8": "p = 0.8",
    "p = 0,9": "p = 0.9",
    "p = 1,0 · kein Abschneiden (A5-Einstellung)": "p = 1.0 · no truncation (the A5 setting)",
    "korrekt · softmax(v/τ), dann kleinste Menge mit Masse ≥ p, dann renormalisieren": "correct · softmax(v/τ), then the smallest set with mass ≥ p, then renormalize",
    "Temperatur auf die Logits, danach Top-p": "temperature on the logits, then top-p",
    "kleinste Menge mit kumulierter Masse ≥ p": "smallest set with cumulative mass ≥ p",
    "durch die behaltene Masse geteilt": "divided by the kept mass",
    "Genau die Gleichungen (23) und (24) des A1-Handouts. Die Temperatur steht im Exponenten, weil sie die Abstände der Logits skalieren soll; das Abschneiden kommt danach, weil V(p) auf der Verteilung definiert ist, aus der wirklich gezogen wird; und die behaltenen Wahrscheinlichkeiten werden durch ihre eigene Summe geteilt, weil eine Verteilung sich zu eins addieren muss. Die Menge enthält immer mindestens ein Token, denn schon das erste erreicht oder überschreitet jede Schwelle p ≤ 1 oder wird ohnehin gebraucht.": "Exactly equations (23) and (24) of the A1 handout. Temperature sits in the exponent because it is meant to scale the spacing of the logits; truncation comes afterwards because V(p) is defined on the distribution that is actually drawn from; and the kept probabilities are divided by their own sum because a distribution has to add up to one. The set always contains at least one token, since the first one already reaches or exceeds any threshold p ≤ 1, or is needed regardless.",
    "Temperatur nach dem Softmax · probs / τ, dann renormalisieren": "temperature after the softmax · probs / τ, then renormalize",
    "Softmax zuerst, Temperatur auf die Wahrscheinlichkeiten": "softmax first, temperature on the probabilities",
    "Alle Wahrscheinlichkeiten durch dieselbe Zahl zu teilen und danach zu renormalisieren ist die Identität – der Faktor 1/τ kürzt sich im Nenner vollständig weg. Die Zeile läuft, wirft nichts und verändert nichts: der Temperaturregler existiert schlicht nicht mehr, und jeder Wert von τ liefert exakt die Zahlen von τ = 1,0. Deshalb ist das der teuerste Fehler in der Ablation von A1 – wer damit Temperaturen vergleicht, misst viermal denselben Lauf. Sichtbar wird er nur gegen eine Referenzverteilung mit τ ≠ 1, und auch dann nicht in einem Kontext, der schon ohne Temperatur auf ein Token zusammenfällt.": "Dividing all probabilities by the same number and renormalizing afterwards is the identity — the factor 1/τ cancels completely in the denominator. The line runs, throws nothing, and changes nothing: the temperature knob simply no longer exists, and every value of τ yields exactly the numbers of τ = 1.0. That makes it the most expensive bug in A1's ablation — anyone comparing temperatures with it measures the same run four times. It only becomes visible against a reference distribution with τ ≠ 1, and even then not in a context that already collapses onto one token without any temperature.",
    "Off-by-one · nur Tokens behalten, solange die Masse unter p bleibt": "off-by-one · keep tokens only while the mass stays below p",
    "größtes Präfix mit kumulierter Masse < p – das überschreitende Token fällt weg": "largest prefix with cumulative mass < p — the crossing token is dropped",
    "Gleichung (24) verlangt die kleinste Menge mit Masse ≥ p, also einschließlich des Tokens, das die Schwelle überschreitet. Wer stattdessen abbricht, bevor die Schwelle erreicht ist, behält eine Menge, deren Masse garantiert kleiner als p ist – und im sicheren Kontext gar keine: dort überschreitet das erste Token bei τ ≤ 1,0 schon allein jede Schwelle p ≤ 0,9, die Menge bleibt leer und torch.multinomial bekommt einen Nullvektor. Der Fehler trifft also genau dann, wenn das Modell sich sicher ist. Und selbst bei p = 1,0, wo Top-p nichts tun soll, verliert er still das unwahrscheinlichste Token.": "Equation (24) asks for the smallest set with mass ≥ p, i.e. including the token that crosses the threshold. Stopping before the threshold is reached instead keeps a set whose mass is guaranteed to fall below p — and in the confident context keeps none at all: there, at τ ≤ 1.0, the first token alone already exceeds every threshold p ≤ 0.9, the set stays empty, and torch.multinomial receives a zero vector. So the bug strikes exactly when the model is confident. And even at p = 1.0, where top-p is supposed to do nothing, it silently loses the least likely token.",
    "Renormalisierung vergessen · behaltene Wahrscheinlichkeiten unverändert lassen": "renormalization forgotten · leave the kept probabilities untouched",
    "keine – die Ausgabe summiert sich zur behaltenen Masse": "none — the output sums to the kept mass",
    "Der Fehler, den man beim Sampling nie bemerkt: torch.multinomial normalisiert seine Gewichte intern, also zieht dieser Code exakt dieselben Tokens mit exakt denselben Häufigkeiten wie die korrekte Version – der Abstand zur korrekten Verteilung ist null. Falsch ist nur die Zahl, die herauskommt: die Ausgabe summiert sich zur behaltenen Masse statt zu eins, und damit ist jede daraus gebildete Log-Wahrscheinlichkeit um genau −ln(Masse) verschoben. Das trifft alles, was Wahrscheinlichkeiten liest statt Tokens zu ziehen: Perplexity, die Entropie im Logging und in A5 get_response_log_probs, wo sich der Versatz über jedes Token der Antwort aufsummiert.": "The bug you never notice while sampling: torch.multinomial normalizes its weights internally, so this code draws exactly the same tokens with exactly the same frequencies as the correct version — the distance to the correct distribution is zero. Only the number that comes out is wrong: the output sums to the kept mass instead of to one, so every log-probability formed from it is shifted by exactly −ln(mass). That hits everything which reads probabilities rather than drawing tokens: perplexity, the entropy in the logs, and in A5 get_response_log_probs, where the offset accumulates over every token of the response.",
    "Reihenfolge vertauscht · erst Top-p auf der rohen Verteilung, dann Temperatur": "order swapped · top-p on the raw distribution first, then temperature",
    "Top-p auf softmax(v), danach Temperatur auf den behaltenen Logits": "top-p on softmax(v), then temperature on the kept logits",
    "kleinste Menge mit Masse ≥ p, gebildet auf der Verteilung bei τ = 1,0": "smallest set with mass ≥ p, formed on the distribution at τ = 1.0",
    "Die Menge V(p) wird hier auf der ungeskalierten Verteilung gebildet und danach erst die Temperatur angewendet. Damit hängt die Größe der Menge nicht mehr von τ ab: eine kalte Temperatur schärft zwar noch die Gewichte innerhalb der Menge, verkleinert sie aber nicht mehr. Das klingt harmlos und ist es bei τ = 1,0 auch – dort stimmen beide Fassungen auf die letzte Stelle überein. Für jedes andere τ verschiebt sich der Zufall am Rand: zu viele Tokens bleiben bei kalter, zu wenige bei heißer Temperatur im Rennen.": "Here V(p) is formed on the unscaled distribution and only then is temperature applied. The size of the set therefore no longer depends on τ: a cold temperature still sharpens the weights inside the set but no longer shrinks it. That sounds harmless, and at τ = 1.0 it is — there both versions agree to the last digit. For every other τ the randomness at the edge shifts: too many tokens stay in the running at a cold temperature, too few at a hot one.",
    "Reihenfolge:": "Order:",
    "Abschneiden:": "Truncation:",
    "Normalisierung:": "Normalization:",
    "Der Vokabular-Ledger": "The vocabulary ledger",
    "acht Kandidaten, nach Wahrscheinlichkeit sortiert. Die Logits sind konstruiert; Softmax, Nucleus und Renormalisierung laufen wirklich über sie.": "eight candidates, sorted by probability. The logits are constructed; softmax, nucleus, and renormalization really do run over them.",
    "kumuliert": "cumulative",
    "ja": "yes",
    "nein": "no",
    "Was daraus folgt": "What follows from it",
    "Größe der Nucleus-Menge |V(p)|": "Size of the nucleus set |V(p)|",
    "von": "of",
    "behaltene Masse vor der Renormalisierung": "kept mass before renormalization",
    "Summe der ausgegebenen Wahrscheinlichkeiten": "Sum of the emitted probabilities",
    "Entropie der Ausgabeverteilung": "Entropy of the output distribution",
    "Wahrscheinlichkeit des wahrscheinlichsten Tokens": "Probability of the most likely token",
    "Wahrscheinlichkeit, dass alle": "Probability that all",
    "Rollouts hier dasselbe Token wählen": "rollouts pick the same token here",
    "Abstand zur korrekten Verteilung (Anteil abweichender Ziehungen)": "Distance to the correct distribution (share of draws that differ)",
    "nicht definiert – es bleibt kein Token übrig": "undefined — no token is left",
    "Masse": "mass",
    "Entropie": "entropy",
    "Es bleibt kein einziges Token übrig.": "Not a single token is left.",
    "Schon das wahrscheinlichste Token überschreitet die Schwelle allein, und eine Regel, die vor dem Überschreiten abbricht, behält deshalb nichts. torch.multinomial bekommt einen Nullvektor und bricht ab – oder liefert, je nach Version, still ein Token aus einer nicht normalisierbaren Verteilung. Genau dieser Fall tritt auf, wenn das Modell sich sicher ist, also mit fortschreitendem Training immer häufiger.": "The most likely token already crosses the threshold on its own, so a rule that stops before crossing keeps nothing. torch.multinomial receives a zero vector and fails — or, depending on the version, silently returns a token from a distribution that cannot be normalized. This case arises exactly when the model is confident, that is, more and more often as training progresses.",
    "Diese Variante zieht hier exakt dieselben Tokens wie die korrekte.": "In this state the variant draws exactly the same tokens as the correct one.",
    "Der Abstand ist null, ein Sampling-Test kann den Fehler in diesem Zustand also nicht sehen. Schalte Kontext, Temperatur oder p durch, bis er auftaucht – und lies in der Kennzahlenliste nach, ob stattdessen die Summe der Ausgabe von eins abweicht.": "The distance is zero, so a sampling test cannot see the bug in this state. Step through context, temperature, or p until it shows up — and check in the metrics list whether the sum of the output departs from one instead.",
    "Das ist der Vertrag aus A1.": "This is the contract from A1.",
    "Temperatur im Exponenten, Nucleus auf der skalierten Verteilung, Renormalisierung durch die behaltene Masse – jede andere Zeile in diesem Menü weicht davon ab, ohne einen Fehler zu werfen.": "Temperature in the exponent, nucleus on the scaled distribution, renormalization by the kept mass — every other line in this menu departs from it without raising an error.",
    "Abweichung zur korrekten Verteilung:": "Departure from the correct distribution:",
    "aller Ziehungen fallen anders aus als sie sollten – ohne Fehlermeldung, ohne Shape-Konflikt, ohne NaN.": "of all draws come out differently than they should — with no error message, no shape conflict, no NaN.",
    "Dieselbe Schwelle, vier Verteilungen": "The same threshold, four distributions",
    "Korrekte Implementierung, gleiche Temperatur, gleiches p – nur der Kontext wechselt.": "Correct implementation, same temperature, same p — only the context changes.",
    "Warum p eine Masse ist und keine Anzahl": "Why p is a mass and not a count",
    "Top-k würde in allen vier Kontexten dieselbe Zahl Tokens behalten und wäre damit im sicheren Kontext viel zu großzügig und im flachen viel zu streng. Top-p fragt stattdessen, wie viel Wahrscheinlichkeitsmasse zusammenkommen muss, und lässt die Verteilung selbst entscheiden, wie viele Tokens dafür nötig sind: bei p = 0,8 und τ = 1,0 ist das im Paris-Kontext ein einziges Token und im flachen Kontext sieben. Genau das ist der Grund, aus dem A1 Nucleus Sampling verlangt und nicht Top-k. Die Temperatur greift eine Stufe davor an: sie verändert die Verteilung, aus der V(p) gebildet wird, und verschiebt damit auch die Größe der Menge – halte p fest und schalte τ durch, dann steht die Kopplung beider Regler in der Spalte |V(p)|.": "Top-k would keep the same number of tokens in all four contexts, making it far too generous in the confident context and far too strict in the flat one. Top-p instead asks how much probability mass has to come together and lets the distribution itself decide how many tokens that takes: at p = 0.8 and τ = 1.0 that is a single token in the Paris context and seven in the flat one. This is precisely why A1 asks for nucleus sampling and not top-k. Temperature acts one step earlier: it changes the distribution from which V(p) is formed, and therefore shifts the size of the set as well — hold p fixed and step through τ, and the coupling of the two knobs stands in the |V(p)| column.",
    "Die drei Antworten trennen dieselben drei Dinge. Erstens: p ist eine Schwelle auf der Wahrscheinlichkeitsmasse, keine Anzahl – deshalb behält dieselbe Zeile Code im sicheren Kontext ein Token und im flachen sieben, und deshalb ist Nucleus Sampling überhaupt etwas anderes als Top-k. Zweitens: eine Zeile, die nichts wirft, kann trotzdem einen ganzen Regler abschalten; die Temperatur nach dem Softmax kürzt sich weg, und eine Ablation über vier Temperaturen misst dann viermal denselben Lauf, ohne dass irgendwo eine Warnung erscheint. Drittens: ein Abstand von null zur korrekten Verteilung heißt nur, dass die gezogenen Tokens stimmen. Fehlt die Renormalisierung, ist jede ausgegebene Wahrscheinlichkeit um denselben Faktor zu klein und jede Log-Wahrscheinlichkeit um −ln(Masse) verschoben – für das Sampling folgenlos, für Perplexity, für das Entropie-Logging und für get_response_log_probs in A5 nicht.": "The three answers separate the same three things. First: p is a threshold on probability mass, not a count — which is why the same line of code keeps one token in the confident context and seven in the flat one, and why nucleus sampling is something other than top-k at all. Second: a line that throws nothing can still switch off an entire knob; temperature after the softmax cancels out, and an ablation over four temperatures then measures the same run four times without any warning appearing anywhere. Third: a distance of zero to the correct distribution only means the drawn tokens are right. If the renormalization is missing, every emitted probability is too small by the same factor and every log-probability is shifted by −ln(mass) — inconsequential for sampling, but not for perplexity, not for the entropy logging, and not for get_response_log_probs in A5.",
    "Alle drei Antworten lassen sich ablesen, keine muss geraten werden. Für die erste stelle p = 0,8 und τ = 1,0 ein und vergleiche die Zeile |V(p)| in „Dieselbe Schwelle, vier Verteilungen“ über alle vier Kontexte. Für die zweite wähle die Variante „Temperatur nach dem Softmax“ und schalte τ durch, ohne sonst etwas zu ändern. Für die dritte wähle „Renormalisierung vergessen“ und lies den Abstand zur korrekten Verteilung und die Summe der Ausgabe in derselben Kennzahlenliste nebeneinander.": "All three answers can be read off; none has to be guessed. For the first, set p = 0.8 and τ = 1.0 and compare the |V(p)| row under “The same threshold, four distributions” across all four contexts. For the second, choose the “temperature after the softmax” variant and step through τ without changing anything else. For the third, choose “renormalization forgotten” and read the distance to the correct distribution and the sum of the output side by side in the same metrics list.",
    "Verschiebung jeder Log-Wahrscheinlichkeit": "shift of every log-probability",
